...
//
// Internally, the grid is represented as byte slices.
func newHeightMap(lines []string) *heightMap {
	rows, columns := len(lines), len(lines[0])

	height := make([][]byte, rows)
	for idx, line := range lines {
		height[idx] = []byte(line)
	}
	...
	scenicScore := make([][]int, rows)
	for i := 0; i < rows; i++ {
		scenicScore[i] = make([]int, columns)
	}
...
func (t *heightMap) VisibleCount() int {
	...
	for r := 1; r < t.rows-1; r++ {
		for c := 1; c < t.columns-1; c++ {
			...
			t.scenicScore[r][c] = viewingDistanceTop * viewingDistanceLeft * viewingDistanceBottom * viewingDistanceRight
...
func (t *heightMap) MaxScore() int {
	maxScore := 0
	for _, scores := range t.scenicScore {
